---
title: "画像異常検知 PatchCoreの実装解説" # 記事のタイトル
...
1. 正常な画像群の特徴マップにおける局所的な部分をパッチ特徴量とする (位置情報を保持した特徴となる)
2. パッチ特徴量のチャンネル方向の次元を削減したものをメモリバンクに保存
3. 高速化のためGreedy法により、メモリバンク内のパッチ特徴量の数を削減

です。推論時には、近傍法を用いて入力画像のパッチ特徴量とメモリバンクに保存されている特徴量の距離を計算し、距離が離れている部分が異常箇所となります。
